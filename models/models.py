...
class Transport(Base):
    ...
    description = Column(String, nullable=True) # описание, может быть null
    ...
    minutePrice = Column(Double, nullable=True)#Цена аренды за минуту (может быть null)
    dayPrice = Column(Double, nullable=True)#Цена аренды за сутки (может быть null)
    ownerId = Column(BigInteger, ForeignKey('usertable.id',ondelete="CASCADE"))
    __table_args__ = (
...
class Rent(Base):
    ...
    transport_id = Column(BigInteger, ForeignKey('transport.id', ondelete="CASCADE"))
    userId = Column(BigInteger, ForeignKey('usertable.id',ondelete="CASCADE"))
    ...
    finalPrice = Column(Double,nullable=True, default=None)
